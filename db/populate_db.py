...
def populate_course_instances(cursor):
    ...
    instances = load_json("db/population/4-instancias_cursos.json")[
        "instancias"
    ]
...
def populate_assessments(cursor, assessments, section):
    for assessment in assessments:
        ...
        VALUES (%s, %s, %s, %s, %s)
        """
        type_evaluate_assessment = section["evaluacion"]["topicos"][
            f"{assessment['id']}"
        ]["tipo"]
        ...
        assessment_data = (
            ...
            section["id"],
            assessment["nombre"],
            type_evaluate(type_evaluate_assessment),
            assessment["valor"],
        )
...
def populate_sections(cursor):
    sections = load_json(
        "db/population/5-instancia_cursos_con_secciones.json"
    )["secciones"]
    for section in sections:
        ...
        data = (
            section["id"],
            section["instancia_curso"],
            section["profesor_id"],
            type_evaluate(section["evaluacion"]["tipo"]),
        )
        insert_data(cursor, query, data)
...
def populate_students_per_section(cursor):
    students_sections = load_json("db/population/6-alumnos_por_seccion.json")[
        "alumnos_seccion"
    ]
...
def get_task_id(cursor, topic_id, instance):
    ...
    if result:
        ...
        print(
            f"No se encontró un task_id para topico_id {topic_id} con instancia {instance}"
        )
        return None
